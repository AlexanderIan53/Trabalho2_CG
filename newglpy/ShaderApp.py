...
class ShaderApp(GLAPP):
    ...
    def drawShader(self):
        ...
        if self.arrayBufferId == None:
            ...
            vertices, uvs, normals, indices = shaderGeometry(4)
...
def shaderGeometry(size=5,segments=10,bottom=True,lid=True,body=True,fitLid=True,blinn=True):
    ...
    for surf in range(minPatches, maxPatches):
        if lid or surf < 20 or surf >= 28:
            for i in range(0,3):
                ...
                for r in range(0,4):
                    for c in range(0,4):
                        gmx[c][r] = shaderVertices[ shaderPatches[ surf * 16 + r * 4 + c ] * 3 + i ]
